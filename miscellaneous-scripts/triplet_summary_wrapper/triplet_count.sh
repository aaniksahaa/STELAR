...
cat $1| xargs -I@ sh -c 'echo -n "@" >'$tmp'; /home/mazharul-islam/Dropbox/Research/STELAR/triplet_summary_wrapper/triplets.soda2103 printTriplets '$tmp';'| sed 's/^/\(/' | sed 's/$/\)\)\;/' | sed 's/ | /,\(/'| sed 's/ /\,/g';
