...
def writeWeightedInputFile (matrix, fileStream, filePrefix = "weighted"):
    ...
    paupBlock = ['begin paup;',
            ...
            '\tcontree all/ strict=yes treefile = %s replace=yes;' % strictConsensusTreeFile,
            '\tcontree all/ majrule=yes strict=no treefile = %s replace=yes;' % majorityConsensusTreeFile,
            '\tcontree all/ majrule=yes strict=no le50=yes treefile = %s replace=yes;' % greedyConsensusTreeFile,
            '\n\tlog stop;',
            '\tquit; end;']
...
def writeWeightedRatchetInputFile (matrix, 
                           fileStream,
                           filePrefix = "ratchet", 
                           numRatchetIterations = 3, 
                           percentToUpweight = .25, 
                           weight = None, 
                           startingTree = None, numChars = None):
    '''
    ...
    for replicate in xrange(numRatchetIterations):
        ...
        weighting='\tweights '
        reweight='\tweights '
        for i in sorted(listOfIndices):
            if str(i) in selectedIndices:
                weighting+='%f: %s' % (weight[i]*2, i)
            else:
                weighting+='%f: %s' % (weight[i], i)
            reweight+='%f: %s' % (weight[i], i)
            if i == (len(listOfIndices)):
                weighting+=';'
                reweight+=';'
            else:
                weighting+=', '
...
def getBestTreeFromRatchet (file):
    '''Return tree with best MP score from the set of all ratchet trees.'''

    f = open (file, 'r')
    lines = f.readlines()
    f.close()

    (bestTree, bestScore) = (None, None)
    readNextTree = False

    trees = []
    for line in lines:
        line = line.strip()
        if line.startswith('>   Score of best tree(s) found'):
            (_, _, score) = line.partition(' = ')
            if bestScore is None or score < bestScore:
                bestScore = score
                #print "tree with score " + score
                readNextTree = True

        if readNextTree and (line.startswith('tree') or line.startswith('Tree') or line.startswith('TREE')):
