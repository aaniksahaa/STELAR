    ...
    implements Cloneable, Serializable 
{
    ...
    private void checkInvariants()
    {
        ...
        if(!$assertionsDisabled && (wordsInUse < 0 || wordsInUse > words.length))
            throw new AssertionError();
        if(!$assertionsDisabled && wordsInUse != words.length && words[wordsInUse] != 0L)
            throw new AssertionError();
        else
            return;
    }
    ...
    public BitSet(int nbits)
    {
        ...
        if(nbits < 0)
        {
            throw new NegativeArraySizeException((new StringBuilder()).append("nbits < 0: ").append(nbits).toString());
        } else
        {
            initWords(nbits);
            sizeIsSticky = true;
    ...
    {
        if(fromIndex < 0)
            throw new IndexOutOfBoundsException((new StringBuilder()).append("fromIndex < 0: ").append(fromIndex).toString());
        if(toIndex < 0)
            throw new IndexOutOfBoundsException((new StringBuilder()).append("toIndex < 0: ").append(toIndex).toString());
        if(fromIndex > toIndex)
            throw new IndexOutOfBoundsException((new StringBuilder()).append("fromIndex: ").append(fromIndex).append(" > toIndex: ").append(toIndex).toString());
        else
            return;
    }
    ...
    public void flip(int bitIndex)
    {
        if(bitIndex < 0)
        {
            throw new IndexOutOfBoundsException((new StringBuilder()).append("bitIndex < 0: ").append(bitIndex).toString());
        } else
        {
    ...
    public boolean get(int bitIndex)
    {
        ...
        } else
        {
            checkInvariants();
            int wordIndex = wordIndex(bitIndex);
            return wordIndex < wordsInUse && (words[wordIndex] & 1L << bitIndex) != 0L;
        }
    }
    ...
    public BitSet get(int fromIndex, int toIndex)
    {
        ...
        long lastWordMask = -1L >>> -toIndex;
        result.words[targetWords - 1] = (toIndex - 1 & 63) >= (fromIndex & 63) ? (words[sourceIndex] & lastWordMask) >>> fromIndex : words[sourceIndex] >>> fromIndex | (words[sourceIndex + 1] & lastWordMask) << -fromIndex;
    ...
    public int length()
    {
        if(wordsInUse == 0)
            ...
            return 64 * (wordsInUse - 1) + (64 - Long.numberOfLeadingZeros(words[wordsInUse - 1]));
    }
    ...
    private static final ObjectStreamField serialPersistentFields[] = {
        new ObjectStreamField("bits", Byte.class)
    };
    private long words[];
...
/*
	...
	Decompiled from: /usr/lib/jvm/java-6-openjdk/jre/lib/rt.jar
	Total time: 106 ms
	Jad reported messages/errors:
The class file version is 49.0 (only 45.3, 46.0 and 47.0 are supported)
Couldn't fully decompile method clone
Couldn't resolve all exception handlers in method clone
	Exit status: 0
	Caught exceptions:
*/
